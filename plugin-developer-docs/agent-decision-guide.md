...
| Persist provider deltas or cursor state | A `records` resource | `resource.apply_delta` or connector pagination | Cursor state must be `host_only` and contain a required string `cursor` field. |
...
- A background or scheduled action is essential. Schema-v2 actions are manual.
